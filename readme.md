...
### Индикация
Текущий уровень мощности индицируется линейкой из пяти (для текущей реализации) светодиодов (для **Arduino** - один красный и четыре зеленых) или двух (для **Attiny45/85** - красный и зеленый); красный светодиод зажигается при выключенной нагрузке;

Вместо красного и первого зеленого светодиодов можно использовать один двухцветный светодиод с общим катодом;

Пины зеленых светодиодов описываются в массиве `led_pins` в файле **header_file.h**;
